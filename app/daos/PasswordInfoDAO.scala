 ...
 */
class PasswordInfoDAO @Inject() (val reactiveMongoApi: ReactiveMongoApi)(implicit ex: ExecutionContext, val classTag: ClassTag[PasswordInfo]) extends DelegableAuthInfoDAO[PasswordInfo] {

  /**
   * The data store for the auth info.
   */
  def passwords = reactiveMongoApi.database.map(_.collection[JSONCollection]("passwords"))
